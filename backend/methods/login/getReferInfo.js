...
module.exports = function(callback, params){/*refer*/
  ...
  makeQuery(`SELECT user_email, user_phone, user_photo, user_blocked FROM users
    WHERE user_phone=? OR user_email=?`, [ refer, refer ],
    res => {
      if( !res.result.length || res.result[0].user_blocked ) return callback( USER_NOT_EXISTS );
      res.result = res.result[0];
      delete res.result.user_blocked;
      var resp = res;
